...
| Changing each other's content | Two game copies, separate mod folders. A player asked the owner for a turn at changing her painting, got her version and her image in the editor, renamed it and sent it back: the owner's own `paintings.json` was rewritten, the owner saw "Bob changed your 'AlphaArt'", and the new version was sent on to everyone. With 'Let others change my content' off, the same request was refused with "They don't let other players change their content." Both directions: the player changed the host's painting and the host changed the player's, each time landing in the owner's own file. With four game copies (own mod folder each, own painting each) everyone showed all four sets, one player changed another player's painting with the host only relaying, and the new version reached all four. Not yet tested by hand: two players asking for the same item at once, a turn running out, and a change refused because the owner edited meanwhile (all three are covered by the code but weren't driven through the UI). |
