...
| Changing the Host's content | Two game copies with separate mod folders (`--mods-path`). Player A joined the Host and took the Host's content (their own saved to a backup zip first). Tested before the model changed to one set per game: content moved both ways and a change landed in the owner's own file, with four copies all seeing each other. The one-set model and the locks that come with it still need the same run: Player A edits, Player B is told who has it, the Host takes a file back, and an earlier version is put back. |
